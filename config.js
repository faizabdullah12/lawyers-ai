...
// ============================================
// 4b. REALTIME CHANNEL MANAGEMENT
// ============================================
window.__ACTIVE_RT_CHANNELS__ = {};
window.__GLOBAL_RT_STATE__ = { connected: navigator.onLine };

window.cleanupRealtime = function() {
    try {
        Object.entries(window.__ACTIVE_RT_CHANNELS__).forEach(([key, channel]) => {
            try {
                channel.unsubscribe();
                console.log('[RT] Cleanup:', key);
            } catch (_) {}
        });
    } catch (err) {
...
window.addEventListener('online', () => {
    console.log('[RT] Network online');
    window.__GLOBAL_RT_STATE__.connected = true;
});

window.addEventListener('offline', () => {
    console.warn('[RT] Network offline');
    window.__GLOBAL_RT_STATE__.connected = false;
});
...
// Escape HTML untuk cegah XSS — versi lebih robust
...
// Sanitize URL untuk cegah javascript: injection
...
// Rate limiter sederhana untuk form submit
...
// Toast pakai CSS vars, bukan Tailwind (konsisten dengan semua halaman)
...
// Tutup sidebar saat tap overlay
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Swipe gesture untuk tutup sidebar di iOS
    let touchStartX = 0;
...
window.requireAuth = async function(redirectTo = 'login.html') {
    try {
        ...
        // Refresh token bila hampir expired (< 5 menit)
...
// Logout bersih — hapus session + channel
